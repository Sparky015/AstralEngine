...
#define ASSERT_SS(expression, errorMessage) [[unlikely]] if (!(expression)) {std::ostringstream ss; ss << (errorMessage); Debug::Macros::macro_ASSERT(#expression, ss, __FILE__, __LINE__, __func__); throw std::runtime_error(ss.str());}
#define STATIC_ASSERT(expression, errorMessage) static_assert(expression, errorMessage);
