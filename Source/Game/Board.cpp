...
Board::Board(const std::string& FEN)
{
    ...
    for (char c : FEN)
    {
        ...
        else // Character is a letter
        {
            ...
            if (c == '/')
            {
                if (squareLocation % 8 == 0) { continue; }
    ...
    for (uint8 pieceID = 0; pieceID < 16; pieceID++)
    {
        ...
        for (uint8 squareNum = 0; squareNum < 64; squareNum++)
        {
            ...
            if (m_Board.ReadSquareType(squareNum) == pieceType && m_Board.ReadSquareColor(squareNum) == BLACK)
            {
                ...
                if (it != m_BlackPieceLocations.end()) {continue; }
...
{
    if (promotionType == PieceType::KING || promotionType == PieceType::PAWN) { throw std::logic_error("Promotion type must be bishop, knight, rook, or queen."); }
    if (ConvertPieceIDToPieceType(pieceID) != PieceType::PAWN) { throw std::logic_error("Piece must be a pawn to promote it."); }
...
PieceID Board::ReadSquarePieceID(uint8_t squareLocation)
{
    ...
    if (pieceType == PieceType::NONE) { throw std::logic_error("There is no piece on square"); }
...
void Board::PrintBoardToConsole()
{
    ...
    {
        if (squareLocation % 8 == 0) {std::cout << "\n"; }

        if (m_Board.ReadSquareColor(squareLocation) == PieceColor::WHITE)
        {
            std::cout << (char)toupper(ConvertPieceTypeToChar(m_Board.ReadSquareType(squareLocation))) << " ";
        }
        else
        {
            std::cout << ConvertPieceTypeToChar(m_Board.ReadSquareType(squareLocation)) << " ";
        }
    }
    std::cout << "\n";
}
