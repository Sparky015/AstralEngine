...
TEST_F(Board_TakePieceTest, IsTakenPieceGone1)
{
    ...
    board.TakePiece(BLACK, PIECE_9, E2);
...
TEST_F(Board_TakePieceTest, IsTakenPieceGone2)
{
    ...
    board.TakePiece(WHITE, PIECE_12, A8);
...
TEST_F(Board_TakePieceTest, IsTakenPieceGone3)
{
    ...
    board.TakePiece(WHITE, PIECE_13, F7);
...
{
    EXPECT_THROW(board.TakePiece(BLACK, PIECE_4, E3), std::logic_error);
}

TEST_F(Board_TakePieceTest, SquareEmpty2)
{
    EXPECT_THROW(board.TakePiece(WHITE, PIECE_13, F5), std::logic_error);
}

TEST_F(Board_TakePieceTest, InvalidMoveLocation1)
{
        EXPECT_THROW(board.TakePiece(BLACK, PIECE_4, -3), std::out_of_range);
}

TEST_F(Board_TakePieceTest, InvalidMoveLocation2)
{
    EXPECT_THROW(board.TakePiece(WHITE, PIECE_13, 64), std::out_of_range);
}

TEST_F(Board_TakePieceTest, OnMoveLocationBounds)
{
    EXPECT_NO_THROW(board.TakePiece(WHITE, PIECE_12, A8));
    EXPECT_EQ(board.ReadSquareType(A8), PieceType::QUEEN);
}